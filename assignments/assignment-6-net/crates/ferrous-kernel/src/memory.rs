...
pub fn setup_kernel_address_space(memory: &mut dyn Memory) -> Result<u32, String> {
    ...
    // 4. Identity Map Network Device MMIO (at 0x3000_0000)
    // Map 1 Page (SimpleNetDevice uses 0x1000 size)
    let net_addr = 0x3000_0000;
    map_page(memory, root_ppn, net_addr, net_addr, PTE_R | PTE_W)?;

    // 5. Stack Mapping for Initial Process
